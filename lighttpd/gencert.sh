...
case $TYPE in
	1)
		if [ "$FILENAME" == "" ]; then
			read -p 'Certificate or domain name [lighttpd]: ' FILENAME
	...
	2)
		if [ "$CAFILENAME" == "" ]; then
			read -p 'CA certificate file name [ca]: ' CAFILENAME
		...
		if [ "$SUBJECT" == "" ]; then
			read -p "Certificate common name [$DOMAIN]: " COMMONNAME
...
ssl.ca-file = \"$THISDIR/$CAFILENAME.pem\""
	...
	3)
		if [ "$FILENAME" == "" ]; then
			read -p 'Certificate name [ca]: ' FILENAME
		...
		if [ "$SUBJECT" == "" ]; then
			read -p "Certificate common name [$(hostname -s)-ca]: " COMMONNAME
